/**
 * Accepted Phase A migration classifications retained for Phase B.
 *
 ...
 * article sources and fails unless both route sets match exactly. Phase C owns the
 * currently unauthorized content paths and must move these fields into canonical
 * content metadata before deleting this file.
 */
...
export function acceptedArticleClassification(route) {
  const classification = ARTICLE_CLASSIFICATION_MANIFEST[route];
  if (!classification) throw new Error(`No accepted article classification exists for ${route}.`);
  return classification;
}

export function assertArticleClassificationParity(routes) {
  const sourceRoutes = [...routes].sort();
  const classificationRoutes = Object.keys(ARTICLE_CLASSIFICATION_MANIFEST).sort();
  if (JSON.stringify(sourceRoutes) !== JSON.stringify(classificationRoutes)) {
    throw new Error(`Article classification parity failed: sources=${sourceRoutes.join(',')} classifications=${classificationRoutes.join(',')}`);
  }
}
