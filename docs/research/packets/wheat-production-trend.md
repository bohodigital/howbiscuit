# U.S. wheat production trend

Packet ID: `wheat-production-trend`
Release: `h31-content-tools-2026-07-24`
Status: **validated / approved**
Reviewer: `howbiscuit-h3.1-work-order`
Generated: 2026-07-24T03:53:57Z
Review due: 2026-10-24T00:00:00Z

## Research question

Summarize annual production with year, unit, and revision caveats.

## Proposed factual claims

### agricultural-statistic

USDA NASS reported final U.S. wheat production of 1,984,537,000 BU for 2025; national production does not establish local store inventory.

Evidence: `nass-wheat-2021`, `nass-wheat-2022`, `nass-wheat-2023`, `nass-wheat-2024`, `nass-wheat-2025`

Limitations: Historical aggregate or identity evidence only; retain the stated geography, period, unit, retrieval date, and source boundary.

## Evidence records

- `nass-wheat-2021`: wheat
- `nass-wheat-2022`: wheat
- `nass-wheat-2023`: wheat
- `nass-wheat-2024`: wheat
- `nass-wheat-2025`: wheat

## Citation-ready source notes

- **usda-nass-quickstats:** nass data retrieved 2026-07-24T03:53:57Z.

## Suggested tables

- `evidence` — U.S. wheat production trend evidence; 5 evidence row(s)

## Suggested charts

- `trend` — U.S. wheat production trend trend; x: `period`, y: `value`

## Freshness and disclosure

Cadence: quarterly. Staleness: current as of 2026-07-24T03:53:57Z.

This static research packet uses approved historical source records and makes no provider call during page load.

- Do not convert aggregate, identity, wholesale, or national production evidence into a live personal bill, shelf price, local availability, or inventory claim.
